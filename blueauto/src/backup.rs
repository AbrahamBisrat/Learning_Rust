/// Bluetooth Auto-connect for Headset: when in range automatically reconnect
use bluer::{Adapter, AdapterEvent, Address, DeviceEvent, DiscoveryFilter, DiscoveryTransport};
use futures::{pin_mut, stream::SelectAll, StreamExt};
use std::{collections::HashSet, env, intrinsics::mir::Discriminant};
...
async fn query_all_device_properties(adapter: &Adapter, addre: Address) -> bluer::Result<()> {
    let device = adapter.device(addr)?;
...
async fn main() -> bluer::Resutt<()> {
    let with_changes = env::args().any(|arg| arg == "--changes");
    let all_properties = env::args().any(|arg| arg == "--all_properties");
